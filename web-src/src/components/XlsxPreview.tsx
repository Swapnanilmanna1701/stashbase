...
import { preparationWaitCopy } from '../preparationCopy';
    ...
    <div className="flex h-full min-h-0 flex-col bg-background" data-testid="xlsx-preview">
      ...
      {failure ? (
        <div className="flex min-h-8 shrink-0 items-center gap-2 border-b border-border bg-background px-3 text-sm text-muted-foreground" role="status">
          ...
          <Button variant="outline" size="xs" disabled={retryBusy} onClick={() => {
            setRetryBusy(true);
            void api.reprocessFile(name, { folder: source?.folder }).catch(() => undefined).finally(() => setRetryBusy(false));
          }}>{retryBusy ? 'Reprocessing…' : 'Reprocess'}</Button>
        </div>
      ) : preparationStatus ? (
