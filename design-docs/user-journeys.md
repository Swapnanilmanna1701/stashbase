...
- A workbook opens read-only with its worksheet order and visible workbook
  features intact; copying cells uses displayed values, while formulas and
  external links remain inert.
...
PDF, DOCX, XLSX, image, audio, supported video, or an explicitly accepted clipboard
...
- Workbook preparation is bounded by container, worksheet, cell, image, output,
  and time limits; a failed extraction never prevents direct read-only preview.
